...
public class UserController {
    ...
    public void changeMail(String token, String email, String oldPassword, String newPassword, ControllerCallback<Payload> callback) {
        String authToken = "Bearer " + token;

        ChangePasswordRequest changePasswordRequest = new ChangePasswordRequest(email, oldPassword, newPassword);
        Call<Payload> call = apiService.changeEmail(authToken, changePasswordRequest);
        call.enqueue(new Callback<Payload>() {
            @Override
            public void onResponse(@NonNull Call<Payload> call, @NonNull Response<Payload> response) {
                if (response.isSuccessful()) {
                    ...
                    if (payload != null) {
                        ...
                        callback.onFailed("Impossibile cambiare l'email");
                    }
                } else {
